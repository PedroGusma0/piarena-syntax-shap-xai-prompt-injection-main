...
`scripts/xai_metrics.py` computes faithfulness metrics (Fidelity(t), acc@1, rank of the injected span) from a `--xai` result file, and renders `shap`-library saliency maps (`shap.plots.text`/`shap.plots.bar`). See [SyntaxSHAP](syntaxshap.md) and `plans/xai-syntaxshap-promptguard.md` for the full design.

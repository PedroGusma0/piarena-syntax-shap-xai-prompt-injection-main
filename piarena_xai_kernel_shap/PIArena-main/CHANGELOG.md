...
- Added the `kernelshap` XAI method ([`piarena/xai/kernelshap/`](piarena/xai/kernelshap/)): explains the `promptguard` classifier's decision using word-level Shapley values computed via `captum.attr.KernelShap`. Loads the classifier directly (`AutoModelForSequenceClassification`, no HF `pipeline()` wrapper) and operates on token-id tensors so a coalition is never re-serialized to text; word-level `feature_mask` grouping comes from the tokenizer's own `.word_ids()`, so subtoken fragmentation (URLs, non-Latin scripts) never inflates the number of explained features. Explains the whole `context` as a single joint coalition game (no per-sentence decomposition — unlike the parallel SyntaxSHAP experiment, Kernel SHAP's cost is a chosen `n_samples` budget, not derived from the text's syntactic structure). Full design in [`plans/xai-kernelshap-promptguard.md`](plans/xai-kernelshap-promptguard.md).
- Added `scripts/xai_metrics.py` / [`piarena/xai/metrics.py`](piarena/xai/metrics.py): computes Fidelity(t)/acc@1 and the injected-span rank percentile from a `--xai kernelshap` result file, and renders `shap.plots.text`/`shap.plots.bar` saliency maps via the official `shap` package (used only for plotting, not for computing the Shapley values themselves).
- Added docs at [`docs/xai/`](docs/xai/) (index + `kernelshap.md`) and an "Add A New XAI Method" section to [`docs/extending.md`](docs/extending.md).
- Added `captum`, `shap`, and `matplotlib` to [`requirements.txt`](requirements.txt) for `--xai kernelshap`.
